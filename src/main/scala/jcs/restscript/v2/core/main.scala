...
object main extends App{
    ...
    val MultTest: Expression = Apply(PrimitiveExp(Mult()), List(new LiteralExp(5), new LiteralExp(10)))

    println(MultTest.eval(emptyEnv, lookup))

    val LetTest = LetExp(List(("x", new LiteralExp(5))), Apply(PrimitiveExp(Mult()), List(VariableExp("x"), VariableExp("x"))))
    println(LetTest.eval(emptyEnv, lookup))

    //val LetTestOutOfScope = VariableExp("x")
    //println(LetTestOutOfScope.eval(emptyEnv, lookup))


    val LambdaTest = LetExp(List(("x", new LiteralExp("XX"))), LambdaExp(List(), VariableExp("x")))
    val LambdaApplyTest = Apply(LambdaTest, List())
    println(LambdaTest.eval(emptyEnv, lookup))
    println(LambdaApplyTest.eval(emptyEnv, lookup))

    val factorialLambda = LambdaExp(List("i"),
                                IfElseExp(VariableExp("i"), Apply(PrimitiveExp(Mult()),
                                    List(VariableExp("i"),
                                        Apply(VariableExp("factorial"),
                                            List(Apply(PrimitiveExp(Add()), List(VariableExp("i"),
                                                new LiteralExp(-1)) : List[Expression]))))),
                                    new LiteralExp(1)))
    val factorial = LetExp(List(("factorial", factorialLambda), ("i", new LiteralExp(10))),
        Apply(VariableExp("factorial"), List(VariableExp("i"))))

    println(factorial.eval(emptyEnv, lookup))
